...
// u=0 → right edge (world-right), u=1 → center
// v=0 → top,  v=1 → bottom
...
interface BDef {
  ...
  rL: string; rR: string;
  chimney: boolean;
  wins: [number, number, number, number][];   // [u, v, dw, dh]
...
function Building({ col, row, def, state }: {
  ...
  // Gable roof: two quads meeting at a horizontal ridge
  ...
  // Chimney box (tiny iso box on left slope)
  const chx = x - W2 * 0.52;
  const chy = ridgeY - 14;
  const chH = 13;
  const chW = 5;
    ...
    <g>
      ...
      {def.chimney && !burning && rh > 0 && (
        <g>
          {/* chimney left face */}
          <polygon
            points={`${chx-chW},${chy+chH/2} ${chx},${chy+chH} ${chx},${chy+chH-chH} ${chx-chW},${chy+chH/2-chH}`}
            fill="#6A4030"
          />
          {/* chimney right face */}
          <polygon
            points={`${chx+chW},${chy+chH/2} ${chx},${chy+chH} ${chx},${chy} ${chx+chW},${chy+chH/2-chH}`}
            fill="#7A5040"
          />
          {/* chimney top */}
          <polygon
            points={`${chx},${chy-chH} ${chx+chW},${chy+chH/2-chH} ${chx},${chy} ${chx-chW},${chy+chH/2-chH}`}
            fill="#8A6050"
          />
          {/* smoke puff */}
      ...
      {!burning && def.wins.map(([u, v, dw, dh], i) => (
        <polygon
          key={i}
          points={wallWindow(x, y, bh, u, v, dw, dh)}
          fill="#FFF3A0"
          opacity={0.88}
          stroke="#C8A830"
          strokeWidth={0.5}
      ...
      {!burning && def.door && (
        <polygon
          points={wallWindow(x, y, bh, def.door[0], def.door[1], def.door[2], def.door[3])}
          fill="#7A5030"
          stroke="#5A3418"
          strokeWidth={0.5}
        />
  ...
  return (
    <section className="bg-[#F5F0E8] py-12">
      ...
      <div className="max-w-7xl mx-auto px-6 mb-6">
        <div className="flex flex-col md:flex-row md:items-end gap-5">
          <div className="shrink-0">
            ...
            <h2
              className="text-2xl md:text-3xl font-bold text-[#1B2A4A]"
          ...
          <div className="flex-1 max-w-lg">
            ...
            <input
              type="range" min={0} max={100} value={scrub}
      ...
      <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-6 items-start">
        ...
        {/* Feed */}
        <div className="w-full lg:w-68 shrink-0 space-y-2 max-h-[440px] overflow-hidden">
          <p className="text-[#6B6560] text-xs font-semibold uppercase tracking-wider mb-3">
            Dernières annonces
          </p>
          {events.length === 0 && (
            <div className="bg-white rounded-xl p-4 text-sm text-[#6B6560] border border-[#EAE3D6]">
          ...
          {events.map((ev) => (
            <div
              key={ev.id}
              className={`bg-white rounded-xl px-4 py-3 shadow-sm border border-[#EAE3D6] ${
                ev.kind === "cession"
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    ...
                    {ev.price !== "—" && (
                      <span className="text-xs font-semibold text-[#C4623A]">{ev.price}</span>
                    )}
                  </div>
                </div>
              </div>
        ...
        <div className="flex-1 min-w-0">
          <svg viewBox="0 0 630 420" className="w-full" style={{ overflow: "visible" }}>
            ...
            {sorted.map(({ id, col, row }) => {
              ...
              return (
                <polygon key={`g-${id}`} points={pts} fill="#54A064" stroke="#3E8850" strokeWidth={0.5} />
              );
            })}
            ...
            {sorted.filter((b) => b.state === "rubble").map((b) => {
              ...
              return (
                <g key={`r-${b.id}`}>
                  ...
                  <rect x={x-9} y={y+TH*0.42} width={6} height={5} fill="#8A4020" rx={0.5}
                    transform={`rotate(18,${x},${y+TH*0.5})`} />
                  <rect x={x+4}  y={y+TH*0.48} width={5} height={4} fill="#6A3018" rx={0.5}
                    transform={`rotate(-12,${x},${y+TH*0.5})`} />
                  <rect x={x-3} y={y+TH*0.55} width={4} height={5} fill="#7A3820" rx={0.5}
                    transform={`rotate(5,${x},${y+TH*0.5})`} />
                </g>
              );
            })}
            ...
            {sorted.filter((b) => b.state !== "rubble").map((b) => {
              const { x, y } = iso(b.col, b.row);
              const originY = y + TH; // scale from base
              return (
                <g
                  key={b.id}
                  style={{
                    transformOrigin: `${x}px ${originY}px`,
                    animation: b.state === "appearing"
                      ? "iso-pop 0.55s cubic-bezier(.22,1,.36,1) forwards"
                      : undefined,
                  }}
                >
            ...
            {priceTags.map((pt) => (
              <g key={pt.id} style={{ animation: "float-price 2.3s ease-out forwards" }}>
                ...
                <text x={pt.x} y={pt.y + 4} textAnchor="middle" fontSize={10} fontWeight="bold" fill="#4ADE80"
          ...
          <div className="flex items-center justify-end gap-2 mt-3">
            ...
            <button
              onClick={() => setSpeed(0)}
              className={`w-8 h-7 rounded text-sm border transition-all ${
                speed === 0
                  ? "bg-[#1B2A4A] text-white border-[#1B2A4A]"
                  : "bg-white text-[#6B6560] border-[#D5CCB8] hover:border-[#1B2A4A]"
              }`}
            >⏸</button>
            {([1, 2, 4] as const).map((s) => (
              <button
                key={s}
                onClick={() => setSpeed(s)}
                className={`px-2.5 h-7 rounded text-xs font-semibold border transition-all ${
                  speed === s
                    ? "bg-[#1B2A4A] text-white border-[#1B2A4A]"
                    : "bg-white text-[#6B6560] border-[#D5CCB8] hover:border-[#1B2A4A]"
                }`}
        ...
        <div className="w-full lg:w-48 shrink-0">
          <div className="bg-[#1B2A4A] rounded-2xl p-5 shadow-xl">
            <p className="text-[#4A6FA8] text-xs font-semibold uppercase tracking-wider mb-4">
              En direct
            </p>
            <div className="mb-4">
              <div
              ...
              <div className="text-[#4A6FA8] text-xs mt-1.5 uppercase tracking-wide">
                transmissions
              </div>
            </div>
            <div className="border-t border-[#2E3F5C] pt-4 mb-5">
              <div
                className="text-3xl font-bold text-[#FF6B6B] tabular-nums leading-none"
                style={{ fontFamily: "var(--font-playfair), Georgia, serif" }}
              >
                {counts.liquidations}
              </div>
              <div className="text-[#A05050] text-xs mt-1.5 uppercase tracking-wide">
                liquidations
              </div>
            </div>
            <a
